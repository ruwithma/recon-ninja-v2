"""Report generator for Recon Ninja v2.
...
<title>Recon Ninja — {{ target }}</title>
<style>
  :root {
    --bg-primary: #1a1a2e;
    --bg-secondary: #16213e;
...
<h1>🥷 Recon Ninja Report — {{ target }}</h1>
...
<footer>
  Generated by <strong>Recon Ninja v2</strong> &mdash; {{ scan_time }}
</footer>
...
def _build_markdown(state: ScanState) -> str:
    ...
    # 1. Header
    lines.append(f"# Recon Ninja Report — {state.target} — {now}\n")
